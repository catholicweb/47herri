...
const filteredItems = computed(() => {
  return allItems.filter((item) => {
    ...
    const matchesfilter = selectedFilter.value === "Todos" || haystack.includes(selectedFilter.value.toLowerCase());
...
<template>
  <div class="not-prose w-full max-w-6xl mx-auto p-4">
    <div class="flex flex-wrap justify-center gap-4 mb-8">
      <button
        ...
        "
        class="px-6 py-2 rounded-full transition-colors duration-200 text-sm font-medium cursor-pointer"
